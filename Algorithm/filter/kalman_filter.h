 ...
 * @brief 通用卡尔曼滤波框架（CMSIS-DSP 矩阵 + 可选自动调整量测维度）
 *
 * 设计思路：
 * =========
 * - 提供一个“矩阵化”的 KF 外壳：状态/观测/控制向量与 P/F/H/Q/R/K 等矩阵由调用方初始化。
 * - 支持在传感器数据有效性变化时动态调整 H/R/K 的维度与数值（UseAutoAdjustment）。
 * - 通过 `User_Func0_f ~ User_Func6_f` 钩子，允许用户替换/扩展标准 KF 流程中的步骤。
 *
 * 平台与内存：
 * =========
 * - 依赖 CMSIS-DSP（`arm_math.h`），并包含 STM32 设备头（当前为 H723）。
 * - `Kalman_Filter_Init()` 内部使用 `user_malloc` 分配大量矩阵与缓冲区。
 *   在 FreeRTOS 环境下默认映射到 `pvPortMalloc`，否则回退到 `malloc`。
 * - 当前未提供释放接口：应按“初始化一次、长期复用”的方式使用，避免重复 init 造成内存泄漏。
 *
 * 线程模型：
 * =========
 * - `KalmanFilter_t` 包含大量可变状态；同一实例不应在多个任务中并发更新。
 */
